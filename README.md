...
Release archives carry GitHub [build-provenance attestations](https://docs.github.com/en/actions/security-for-github-actions/using-artifact-attestations) generated by the release workflow. When you have an authenticated [`gh`](https://cli.github.com/) CLI on your `PATH`, `install.sh` verifies the archive against its attestation after the checksum check and aborts if verification fails. Without `gh` the check is skipped; verify manually with:

```bash
gh attestation verify vibeusage_<os>_<arch>.tar.gz --repo joshuadavidthomas/vibeusage
```

Set `VIBEUSAGE_REQUIRE_ATTESTATION=1` to make the script fail when attestation verification is unavailable. Attestations exist only for releases cut after v0.12.0.
